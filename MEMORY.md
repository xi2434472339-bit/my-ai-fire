# MEMORY

## 项目记忆

- 项目名称：销售台账 (sales-ledger)
- 云端同步使用腾讯云 CloudBase，环境 ID: sales-ledger-d9gp6k3tz489082a1
- Firebase 代码存在于 src/lib/firebase.ts 但未被实际使用（死代码）
- 密码存储在环境变量 VITE_ACCESS_PASSWORD 中
- 种子数据 SEED_RECORDS 为空数组
- 已结账状态可通过编辑记录改回未结
- 本地开发默认禁用云同步（VITE_ENABLE_CLOUD_SYNC=false），避免污染正式数据
- 云同步闸门位于 tcb.ts 的 isTcbConfigured()

## 关键设计决策

- 软删除：记录删除后标记 deletedAt，30 天自动清理。sanitizeRecord 必须保留 deletedAt
- 同步策略：5 秒轮询 + updatedAt 时间戳判断
- hydrateFromCloud 全量替换 records，依赖 sanitizeRecord 正确保留所有字段
- Dashboard 汇总：仅统计已选中记录
- 环境安全：VITE_APP_ENV=development 时强制禁用云同步

## 已知陷阱

- sanitizeRecord 返回的字段必须与 LedgerRecord 对齐，漏掉字段会导致 pull 时数据丢失
